package com.community.project.system.result.controller;
...
import com.community.project.system.answer.service.IScaleAnswerService;
...
import com.community.project.system.dept.domain.Dept;
import com.community.project.system.dept.service.IDeptService;
import com.community.project.system.factor.domain.ScaleFactor;
import com.community.project.system.factor.service.IScaleFactorService;
...
import com.community.project.system.question.service.IScaleQuestionService;
import com.community.project.system.result.domain.ScaleResult;
import com.community.project.system.result.service.IScaleResultService;
...
//       根据键，获取键所对应的值。方法提示:get(K key)
        ...
        Optional<Map.Entry<String, Double>> maxEntry = factorAverageMap.entrySet()
                .stream()
                .max(Map.Entry.comparingByValue());

        String jobType = "";
        Double jobScore = 0.00;
        if (maxEntry.isPresent()) {
            jobType = maxEntry.get().getKey();
            jobScore = maxEntry.get().getValue();
            System.out.println("Key with max value: " + maxEntry.get().getKey());
            System.out.println("Max value: " + maxEntry.get().getValue());
        } else {
        ...
        // 用户id
        scaleUserResult.setUserId(user.getUserId());
        scaleUserResult.setLoginName(user.getLoginName());
        scaleUserResult.setUserName(user.getUserName());
        // 测评量表id
        scaleUserResult.setScaleId(scale.getScaleId());
        // 测评量表
        scaleUserResult.setScaleTitle(scale.getScaleTitle());
        // 总分
        scaleUserResult.setCountScore(countScore);
        // 因子分数
        scaleUserResult.setFactorScore(factorResultShow);
        //  结果提示
        if (result!=null){      scaleUserResult.setResultContent(result.getResultContent());  }
        if (classes!=null){    scaleUserResult.setClassesName(classes.getClassesName());   }

        // 因子结果提示
        String suggestJob = jobType +"均分最高：" + jobScore + ",所以您最适合做：" + jobType +" ；类型的工作，具体工作请参考对应类型工作。";
        // scaleUserResult.setFactorResult(factorResult);
        scaleUserResult.setFactorResult(suggestJob);
        // 用户问题选择答案
        scaleUserResult.setUserAnswer(userAnswer);
